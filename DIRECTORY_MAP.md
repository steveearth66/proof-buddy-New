...
| File | Purpose |
|---|---|
| `models.py` | `Term` (course section), `Assignment` (problem set with due date), `AssignmentSubmission` (student work linked to proofs with a grade float). `Term` has an instructor and a set of student `Account` references. |
