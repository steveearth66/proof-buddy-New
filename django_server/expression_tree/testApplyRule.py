...
test_strings = [
    # ['Can only apply the cons rule to first and rest']),
    ...
    # expected error 'argument #2 of cons must be a list'
    ('consList', "(cons 2 3)"),
    ('consList', "(cons 2 '(cons (2 '(3))))"),  # expected '(2 cons (2 '(3)))
    ('consList', "(cons '() '(y z a))"),  # expected '(null y z a)
    # expected error 'cons expects 2 arguments, 1 provided'
    ('consList', "(cons 2)"),
    # expected error 'cons expects 2 arguments, 1 provided'
    ('consList', "(cons null)"),
    # expected error 'cons expects 2 arguments, 3 provided'
    ('consList', "(cons 2 3 null)"),
    # expected error 'cons takes in types [ANY, LIST]'
    ('consList', "(cons null 2)"),
...
for rule, expr in test_strings:
    ...
    print("after rule =", proof.getPrevRacket()
          if proof.errLog == [] else proof.errLog)
