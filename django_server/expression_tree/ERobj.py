...
class ERobj:
    def __init__(self, name, pbType: Type, ins=None, outType=None, value=None, numArgs=None, length=None):
        ...
        # for ints/bools/lists this is a wrapper. for functions it's the lambda expression of it's def.
        self.value = value
...
padd = ERobj("+", Type.FUNCTION, (Type.INT, Type.INT),
             Type.INT, lambda x, y: x+y, 2)
psubtr = ERobj("-", Type.FUNCTION, (Type.INT, Type.INT),
               Type.INT, lambda x, y: x-y, 2)
pmult = ERobj("*", Type.FUNCTION, (Type.INT, Type.INT),
              Type.INT, lambda x, y: x*y, 2)
...
pgtr = ERobj(">", Type.FUNCTION, (Type.INT, Type.INT),
             Type.BOOL, lambda x, y: x > y, 2)
pgtreq = ERobj(">=", Type.FUNCTION, (Type.INT, Type.INT),
               Type.BOOL, lambda x, y: x >= y, 2)
pless = ERobj("<", Type.FUNCTION, (Type.INT, Type.INT),
              Type.BOOL, lambda x, y: x < y, 2)
plesseq = ERobj("<=", Type.FUNCTION, (Type.INT, Type.INT),
                Type.BOOL, lambda x, y: x <= y, 2)
pquotient = ERobj("quotient", Type.FUNCTION, (Type.INT,
                  Type.INT), Type.INT, lambda x, y: x//y, 2)
prem = ERobj("remainder", Type.FUNCTION, (Type.INT, Type.INT),
             Type.INT, lambda x, y: x % y, 2)
pand = ERobj("and", Type.FUNCTION, (Type.BOOL, Type.BOOL),
             Type.BOOL, lambda x, y: x and y, 2)
por = ERobj("or", Type.FUNCTION, (Type.BOOL, Type.BOOL),
            Type.BOOL, lambda x, y: x or y, 2)
pnot = ERobj("not", Type.FUNCTION, (Type.BOOL,), Type.BOOL, lambda x: not x, 1)
pxor = ERobj("xor", Type.FUNCTION, (Type.BOOL, Type.BOOL),
             Type.BOOL, lambda x, y: x != y, 2)
pimp = ERobj("implies", Type.FUNCTION, (Type.BOOL, Type.BOOL),
             Type.BOOL, lambda x, y: (not x) or y, 2)
pnullPred = ERobj("null?", Type.FUNCTION, (Type.ANY,),
...
plistPred = ERobj("list?", Type.FUNCTION, (Type.ANY,),
                  Type.BOOL, lambda x: isinstance(x, list), 1)
pnull = ERobj("null", Type.LIST, None, None, [], None,
              0)  # first time len attrib not None
...
plambda = ERobj("\u03BB", Type.FUNCTION,
                (Type.LIST, Type.ANY), Type.FUNCTION, None, 2)
...
pif = ERobj("if", Type.FUNCTION, (Type.BOOL,
            Type.ANY, Type.ANY), Type.ANY, None, 3)
...
# can use perr.value to store a string of specific error msg
perr = ERobj("Error", Type.ERROR)
