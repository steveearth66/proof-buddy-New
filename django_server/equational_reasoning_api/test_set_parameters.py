"""
Tests for the Set Parameters feature on EquationalProof.

Covers:
  Step 1: Model has 6 support param fields with BooleanField(default=True)
  Step 2: PATCH /api/v1/equational/set-parameters endpoint access control and behaviour
  Step 2: GET /api/v1/equational/get-proof-lines response includes the 6 param fields

DO NOT register in proofs/tests.py until Steps 1 and 2 are implemented.

Run individually:
  $env:PYTHONIOENCODING="utf-8"; py manage.py test equational_reasoning_api.test_set_parameters
"""
...
class EquationalProofParamModelTests(TestCase):
    """Verify EquationalProof model has all 6 support param fields defaulting to True."""
...
class EquationalSetParametersAPITests(TestCase):
    """Verify PATCH /api/v1/equational/set-parameters endpoint."""
    ...
    def test_instructor_can_set_all_fields_low(self):
        ...
        response = self.instr_client.patch(
            '/api/v1/equational/set-parameters', payload, format='json'
        )
    ...
    def test_student_is_forbidden(self):
        ...
        student_proof = EquationalProof.objects.create(
            user=self.student,
            lhs_goal='(+ 1 2)',
            rhs_goal='3'
        )
    ...
    def test_unauthenticated_is_rejected(self):
        ...
        response = unauth.patch(
            '/api/v1/equational/set-parameters', self._payload(), format='json'
        )
    ...
    def test_get_proof_lines_includes_param_fields(self):
        """get-proof-lines response includes all 6 support param fields when proof is in cache."""
        ...
        cache.set(
            ...
            {'proof_obj': dumps(proof_obj), 'proof_id': self.proof.id},
            timeout=None
        )

        response = self.instr_client.get('/api/v1/equational/get-proof-lines')
        self.assertEqual(response.status_code, 200)

        if response.data.get('hasProof'):
            for field in PARAM_FIELDS:
                self.assertIn(
                    field, response.data,
                    f"get-proof-lines response missing support param field: {field}"
                )
